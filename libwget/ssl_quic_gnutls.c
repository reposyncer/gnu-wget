...
static int tp_send_func(gnutls_session_t session, gnutls_buffer_t extdata)
{
	...
	if (n_encoded < 0) {
		wget_debug_printf("ngtcp2_encode_transport_params: %s",
				  ngtcp2_strerror(n_encoded));
		return -1;
	}

	ret = gnutls_buffer_append_data(extdata, buf, n_encoded);
	if (ret < 0) {
		wget_debug_printf("gnutls_buffer_append_data failed: %s",
				  gnutls_strerror(ret));
		return -1;
	}
