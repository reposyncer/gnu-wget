	...
	void **con_cls WGET_GCC_UNUSED)
{
	...
	// for (int i = 0; i < wget_vector_size(requests_made); i++) {
	// 	wget_test_request_t *req = wget_vector_get(requests_made, i);
	// 	wget_debug_printf("[2] Request: %s %s\n", req->method, req->identifier);
	// }
	...
	for (proto_pass = 0; proto_pass < END_PASS; proto_pass++) {
		...
		for (key = first_key; key; key = va_arg(args, int)) {
			...
			case WGET_TEST_EXPECTED_REQUESTS:
				...
				// 	expected_requests = wget_vector_create(8, NULL);
				// 	wget_vector_set_destructor(expected_requests, NULL);
				// }
				if (requests_made) {
